...
class snapshot_header:
  def __init__(self, filename, cosmic_ray_species=0):
    ...
    self.stellar = (np.fromfile(f,dtype=np.int32,count=1))[0]
    self.metals = (np.fromfile(f,dtype=np.int32,count=1))[0]
    self.nallhw = np.fromfile(f,dtype=np.uint32,count=6)
...
def read_block(filename, block, parttype=-1, physical_velocities=True, arepo=0, no_masses=False, verbose=False, cosmic_ray_species=0, indices_choose=0):
  if (verbose):
  ...
  for i in range(filenum): # main loop over files
    ...
    if format in [1,2]: # ----- read from format 1 or 2 snapshot -----
      ...
      for j in range(6):
        if actual_data_for_type[j]:
          if block=="MASS" and massarr[j]>0: # add mass block for particles for which the mass is specified in the snapshot header
            ...
            if parttype>=0:
              if block == 'POS ':
                idx_pos = (curdat[:,0] > 0) & (curdat[:,0] < 100) & (curdat[:,1] > 0) & (curdat[:,1] < 100) & (curdat[:,2] > 0) & (curdat[:,2] < 100)
                inds_pos.append(idx_pos)
              else:
                idx_pos = indices_choose[i]
              data.extend(curdat[idx_pos])
            else:
  ...
  return data, inds_pos

#============
#select data
if __name__ == "__main__":
    dm_pos, idx_pos = read_block("/home/cossim/Jiutian/M1000/simu/snapdir_127/snapshot_converted_127", "POS ", parttype=1)
    dm_pos = np.array(dm_pos, dtype = np.float32)
    dm_vel, idx_ = read_block("/home/cossim/Jiutian/M1000/simu/snapdir_127/snapshot_converted_127", "VEL ", parttype=1, indices_choose = idx_pos)
    dm_vel = np.array(dm_vel, dtype = np.float32)
    print(dm_pos.shape, dm_vel.shape)

    filename = '../testbox100/data/sample_100Mpc_snapshot.hdf5'
    d5 = h5py.File(filename,'w')
    d5.create_dataset('POS', data = dm_pos, dtype = np.float32)
    d5.create_dataset('VEL', data = dm_vel, dtype = np.float32)
    d5.close()